package com.ajie.pay.unionpay.acp.sdk.service.impl;

import com.ajie.pay.unionpay.acp.sdk.*;
...
//        String txnTime = req.getParameter("txnTime");
        ...
        /***商户接入参数***/
        requestData.put("merId", "777290058179213");    	          			  //商户号码，请改成自己申请的正式商户号或者open上注册得来的777测试商户号
        requestData.put("accessType", "0");             			  //接入类型，0：直连商户
        requestData.put("orderId","1575122833636666666666");             //商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
        requestData.put("txnTime", DemoBase.getCurrentTime());        //订单发送时间，取系统时间，格式为yyyyMMddHHmmss，必须取当前时间，否则会报txnTime无效
        requestData.put("currencyCode", "156");         			  //交易币种（境内商户一般是156 人民币）
        requestData.put("txnAmt", "99999");             			      //交易金额，单位分，不要带小数点
...
//        resp.getWriter().write(html);
    ...
    @Override
    public String backRecive(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        System.out.println("##############订单入库操作+订单号："+reqParam.get("orderId"));
        return null;
    }

    @Override
    public String frontRecive(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
    ...
    @Override
    public String refound(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        /***商户接入参数***/
        data.put("merId", "777290058179213");                //商户号码，请改成自己申请的商户号或者open上注册得来的777商户号测试
        data.put("accessType", "0");                         //接入类型，商户接入固定填0，不需修改
        data.put("orderId", "157513332334222");          //商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则，重新产生，不同于原消费
        data.put("txnTime", DemoBase.getCurrentTime());      //订单发送时间，格式为yyyyMMddHHmmss，必须取当前时间，否则会报txnTime无效
        data.put("currencyCode", "156");                     //交易币种（境内商户一般是156 人民币）
        data.put("txnAmt", "999");                          //****退货金额，单位分，不要带小数点。退货金额小于等于原消费金额，当小于的时候可以多次退货至退货累计金额等于原消费金额
        ...
        /***要调通交易以下字段必须修改***/
        data.put("origQryId", "172003112031166790248");      //****原消费交易返回的的queryId，可以从消费交易后台通知接口中或者交易状态查询接口中获取
...
//        String txnTime = null; //订单发送时间
        ...
        /***要调通交易以下字段必须修改***/
        data.put("orderId", "1575122833636666666666");                 //****商户订单号，每次发交易测试需修改为被查询的交易的订单号
